// Type declarations for portable-mcp-client/client-lib.mjs
// Minimal, stable surface consumed by tests. Keep in sync with implementation.
...
export interface CrudSummary {
  ...
  validated: number;
  removed: number;
  failures: CrudFailure[];
  durationMs: number;
  ok?: boolean;
}
export interface RunCrudScenarioOptions {
  command?: string;
  args?: string[];
}
export interface RunCrudScenarioFlags {
  verbose?: boolean;
  json?: boolean;
...
export interface InstructionClient {
  create(entry: InstructionCreateInput, opts?: { overwrite?: boolean }): Promise<Record<string, unknown>>;
  read(id: string): Promise<Record<string, unknown>>;
  update(entry: InstructionCreateInput): Promise<Record<string, unknown>>;
  remove(id: string): Promise<Record<string, unknown> | { removed: boolean }>;
  list(): Promise<{ items: Array<Record<string, unknown>>; count: number; hash?: string }>;
  close(): Promise<void>;
  dispatcher: boolean;
}
...
export interface CrudSequenceResult {
  ...
  read1: Record<string, unknown>;
  updated: Record<string, unknown>;
  read2: Record<string, unknown>;
  removed: Record<string, unknown> | { removed: boolean };
  failures: CrudFailure[];
  ok?: boolean;
}
