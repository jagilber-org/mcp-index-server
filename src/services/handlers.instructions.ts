...
import { computeGovernanceHash, ensureLoaded, invalidate, projectGovernance } from './catalogContext';
...
registerHandler('instructions/import', guard('instructions/import', (p:{entries:ImportEntry[]; mode?:'skip'|'overwrite'})=>{ const entries=p.entries||[]; const mode=p.mode||'skip'; if(!Array.isArray(entries)||!entries.length) return { error:'no entries' }; const dir=path.join(process.cwd(),'instructions'); if(!fs.existsSync(dir)) fs.mkdirSync(dir,{recursive:true}); let imported=0, skipped=0, overwritten=0; const errors: { id:string; error:string }[]=[]; const classifier=new ClassificationService(); for(const e of entries){ if(!e || !e.id || !e.title || !e.body){ const id=(e as Partial<ImportEntry>)?.id||'unknown'; errors.push({ id, error:'missing required fields'}); continue; } const file=path.join(dir, `${e.id}.json`); if(fs.existsSync(file) && mode==='skip'){ skipped++; continue; } if(fs.existsSync(file) && mode==='overwrite') overwritten++; else if(!fs.existsSync(file)) imported++; const now=new Date().toISOString(); const categories=Array.from(new Set((Array.isArray(e.categories)? e.categories: []).filter((c):c is string => typeof c==='string').map(c=>c.toLowerCase()))).sort(); const newBodyHash=crypto.createHash('sha256').update(e.body,'utf8').digest('hex'); let existing:InstructionEntry|null=null; if(fs.existsSync(file)){ try { existing=JSON.parse(fs.readFileSync(file,'utf8')); } catch { existing=null; } } const base: InstructionEntry = existing ? { ...existing, title:e.title, body:e.body, rationale:e.rationale, priority:e.priority, audience:e.audience, requirement:e.requirement, categories, updatedAt: now } as InstructionEntry : { id:e.id, title:e.title, body:e.body, rationale:e.rationale, priority:e.priority, audience:e.audience, requirement:e.requirement, categories, sourceHash:newBodyHash, schemaVersion:'1', deprecatedBy:e.deprecatedBy, createdAt:now, updatedAt:now, riskScore:e.riskScore } as InstructionEntry; if(existing && existing.sourceHash && existing.sourceHash!==newBodyHash){ const parts=(existing.version||'1.0.0').split('.').map(n=>parseInt(n||'0',10)); while(parts.length<3) parts.push(0); parts[2]+=1; base.version=parts.join('.'); base.changeLog=[...(existing.changeLog||[]), { version: base.version, changedAt: now, summary:'body changed (auto bump)' }]; base.sourceHash=newBodyHash; } const sourceHash=base.sourceHash||newBodyHash; base.sourceHash=sourceHash; const record=classifier.normalize(base); if(record.owner==='unowned'){ const auto=resolveOwner(record.id); if(auto){ record.owner=auto; record.updatedAt=new Date().toISOString(); } } try { atomicWriteJson(file, record); } catch { errors.push({ id:e.id, error:'write-failed'}); } } invalidate(); const st=ensureLoaded(); return { hash: st.hash, imported, skipped, overwritten, total: entries.length, errors }; }));

registerHandler('instructions/add', guard('instructions/add', (p:{ entry: Partial<ImportEntry>; overwrite?: boolean; lax?: boolean })=>{ const e=p.entry||{}; const lax=!!p.lax; if(!e.id || !e.body){ if(!lax) return { error:'missing required id/body' }; } if(lax){ if(!e.title) e.title=e.id||'untitled'; if(typeof e.priority!=='number') e.priority=50; if(!e.audience) e.audience='all'; if(!e.requirement) e.requirement='optional'; if(!Array.isArray(e.categories)) e.categories=[]; } if(!e.id || !e.title || !e.body || typeof e.priority!=='number' || !e.audience || !e.requirement){ return { error:'missing required fields', id:e.id }; } const dir=path.join(process.cwd(),'instructions'); if(!fs.existsSync(dir)) fs.mkdirSync(dir,{recursive:true}); const file=path.join(dir, `${e.id}.json`); const exists=fs.existsSync(file); const overwrite=!!p.overwrite; if(exists && !overwrite){ const st0=ensureLoaded(); return { id:e.id, skipped:true, created:false, overwritten:false, hash: st0.hash }; } const now=new Date().toISOString(); const categories=Array.from(new Set((Array.isArray(e.categories)? e.categories: []).filter((c):c is string=> typeof c==='string').map(c=>c.toLowerCase()))).sort(); const sourceHash=crypto.createHash('sha256').update(e.body,'utf8').digest('hex'); const classifier=new ClassificationService(); let base: InstructionEntry; if(exists){ try { const existing=JSON.parse(fs.readFileSync(file,'utf8')) as InstructionEntry; base={ ...existing, title:e.title, body:e.body, priority:e.priority, audience:e.audience, requirement:e.requirement, categories, updatedAt: now }; if(existing.sourceHash!==sourceHash){ const parts=(existing.version||'1.0.0').split('.').map(n=>parseInt(n||'0',10)); while(parts.length<3) parts.push(0); parts[2]+=1; base.version=parts.join('.'); base.changeLog=[...(existing.changeLog||[]), { version: base.version, changedAt: now, summary:'body changed (auto bump)' }]; base.sourceHash=sourceHash; } } catch { base={ id:e.id, title:e.title, body:e.body, rationale:e.rationale, priority:e.priority, audience:e.audience, requirement:e.requirement, categories, sourceHash, schemaVersion:'1', deprecatedBy:e.deprecatedBy, createdAt: now, updatedAt: now, riskScore:e.riskScore } as InstructionEntry; } } else { base={ id:e.id, title:e.title, body:e.body, rationale:e.rationale, priority:e.priority, audience:e.audience, requirement:e.requirement, categories, sourceHash, schemaVersion:'1', deprecatedBy:e.deprecatedBy, createdAt: now, updatedAt: now, riskScore:e.riskScore } as InstructionEntry; } const record=classifier.normalize(base); if(record.owner==='unowned'){ const auto=resolveOwner(record.id); if(auto){ record.owner=auto; record.updatedAt=new Date().toISOString(); } } try { atomicWriteJson(file, record); } catch(err){ return { id:e.id, error:(err as Error).message||'write-failed' }; } invalidate(); const st=ensureLoaded(); return { id:e.id, created: !exists, overwritten: exists && overwrite, skipped:false, hash: st.hash }; }));

registerHandler('instructions/remove', guard('instructions/remove', (p:{ ids:string[]; missingOk?: boolean })=>{ const ids=Array.isArray(p.ids)? Array.from(new Set(p.ids.filter(x=> typeof x==='string' && x.trim()))):[]; if(!ids.length) return { removed:0, missing:[], errors:['no ids supplied'] }; const base=path.join(process.cwd(),'instructions'); const missing:string[]=[]; const removed:string[]=[]; const errors:{ id:string; error:string }[]=[]; for(const id of ids){ const file=path.join(base, `${id}.json`); try { if(!fs.existsSync(file)){ missing.push(id); continue; } fs.unlinkSync(file); removed.push(id); } catch(e){ errors.push({ id, error: e instanceof Error? e.message: 'delete-failed' }); } } if(removed.length){ invalidate(); ensureLoaded(); } return { removed: removed.length, removedIds: removed, missing, errorCount: errors.length, errors }; }));
...
registerHandler('instructions/enrich', guard('instructions/enrich', ()=>{
  ...
  const baseDir=path.join(process.cwd(),'instructions');
  ...
  if(rewritten){ invalidate(); ensureLoaded(); }
...
registerHandler('instructions/repair', guard('instructions/repair', (_p:unknown)=>{ const st=ensureLoaded(); const toFix: { entry: InstructionEntry; actual:string }[]=[]; for(const e of st.list){ const actual=crypto.createHash('sha256').update(e.body,'utf8').digest('hex'); if(actual!==e.sourceHash) toFix.push({ entry:e, actual }); } if(!toFix.length) return { repaired:0, updated:[] }; const repaired:string[]=[]; for(const { entry, actual } of toFix){ const file=path.join(process.cwd(),'instructions', `${entry.id}.json`); try { const updated={ ...entry, sourceHash: actual, updatedAt:new Date().toISOString() }; fs.writeFileSync(file, JSON.stringify(updated,null,2)); repaired.push(entry.id); } catch { /* ignore */ } } if(repaired.length){ invalidate(); ensureLoaded(); } return { repaired: repaired.length, updated: repaired }; }));
...
registerHandler('instructions/groom', guard('instructions/groom', (p:{ mode?: { dryRun?: boolean; removeDeprecated?: boolean; mergeDuplicates?: boolean; purgeLegacyScopes?: boolean } })=>{
  ...
  if(purgeLegacyScopes){ const baseDir=path.join(process.cwd(),'instructions'); for(const e of byId.values()){ const filePath=path.join(baseDir, `${e.id}.json`); try { if(fs.existsSync(filePath)){ const raw=JSON.parse(fs.readFileSync(filePath,'utf8')) as { categories?: unknown[] }; if(Array.isArray(raw.categories)){ const legacyTokens=raw.categories.filter(c=> typeof c==='string' && /^scope:(workspace|user|team):/.test(c)); if(legacyTokens.length){ purgedScopes += legacyTokens.length; updated.add(e.id); } } } } catch { /* ignore */ } } if(dryRun && purgedScopes) notes.push(`would-purge:${purgedScopes}`); }
  { const baseDir=path.join(process.cwd(),'instructions'); for(const e of byId.values()){ const filePath=path.join(baseDir, `${e.id}.json`); let storedHash=e.sourceHash||''; try { if(fs.existsSync(filePath)){ const raw=JSON.parse(fs.readFileSync(filePath,'utf8')) as { sourceHash?:string }; if(typeof raw.sourceHash==='string') storedHash=raw.sourceHash; } } catch(_err){ /* ignore read error */ } const actualHash=crypto.createHash('sha256').update(e.body,'utf8').digest('hex'); if(storedHash!==actualHash){ const parts=(e.version||'1.0.0').split('.').map(n=>parseInt(n||'0',10)); while(parts.length<3) parts.push(0); parts[2]+=1; const newVersion=parts.join('.'); e.changeLog=[...(e.changeLog||[]), { version:newVersion, changedAt:new Date().toISOString(), summary:'hash repaired (auto bump)' }]; e.version=newVersion; e.sourceHash=actualHash; repairedHashes++; e.updatedAt=new Date().toISOString(); updated.add(e.id); } } }
  deprecatedRemoved = toRemove.length; if(!dryRun){ const baseDir=path.join(process.cwd(),'instructions'); for(const id of toRemove){ byId.delete(id); } for(const id of updated){ if(!byId.has(id)) continue; const e=byId.get(id)!; try { fs.writeFileSync(path.join(baseDir, `${id}.json`), JSON.stringify(e,null,2)); filesRewritten++; } catch(err){ notes.push(`write-failed:${id}:${(err as Error).message}`); } } for(const id of toRemove){ try { fs.unlinkSync(path.join(baseDir, `${id}.json`)); } catch(err){ notes.push(`delete-failed:${id}:${(err as Error).message}`); } } if(updated.size || toRemove.length){ invalidate(); ensureLoaded(); } } else { if(updated.size) notes.push(`would-rewrite:${updated.size}`); if(toRemove.length) notes.push(`would-remove:${toRemove.length}`); }
