...
function findResponse(lines: string[], id: number): RpcResponse | undefined {
  for(const l of lines){
    try { const obj = JSON.parse(l) as RpcResponse; if(obj && obj.id===id) return obj; } catch { /* ignore */ }
  }
  return undefined;
}
...
describe('instructions/add overwrite version semantics (alpha)', () => {
  ...
  it('retains existing version & changeLog when body changes without explicit version; updates only when provided', async () => {
    ...
    send(server,{ jsonrpc:'2.0', id:10, method:'instructions/add', params:{ entry:{ id, title:id, body:'Initial body', priority:55, audience:'all', requirement:'optional', categories:['alpha'], version:'2.5.0', owner:'alpha-owner', priorityTier:'P2', semanticSummary:'Initial summary' }, overwrite:true, lax:true } });
  ...
  const first = JSON.parse(fs.readFileSync(file,'utf8')) as { version:string; changeLog: { version:string; changedAt:string; summary:string }[]; owner:string; priorityTier:string; semanticSummary:string };
    ...
    send(server,{ jsonrpc:'2.0', id:11, method:'instructions/add', params:{ entry:{ id, title:id, body:'Modified body once', priority:55, audience:'all', requirement:'optional', categories:['alpha'] }, overwrite:true, lax:true } });
  ...
  const second = JSON.parse(fs.readFileSync(file,'utf8')) as { version:string; changeLog: { version:string; changedAt:string; summary:string }[]; body:string };
    ...
    send(server,{ jsonrpc:'2.0', id:12, method:'instructions/add', params:{ entry:{ id, title:id, body:'Modified body twice', priority:55, audience:'all', requirement:'optional', categories:['alpha'], version:'2.6.0' }, overwrite:true, lax:true } });
    await waitFor(()=> !!findResponse(out,12));
