...
describe('mutation paths', () => {
  it('exercises add/remove/groom with mutation enabled', async () => {
    ...
    // add
  proc.stdin?.write(JSON.stringify({ jsonrpc:'2.0', id:2, method:'tools/call', params:{ name:'instructions/add', arguments:{ entry:{ id:'x1', body:'hello', title:'X1', priority:5, audience:'all', requirement:'optional', categories:['Test'] }, overwrite:true } } })+'\n');
  await waitFor(out, o=> o.id===2 && o.result !== undefined);
    // groom dry run
  proc.stdin?.write(JSON.stringify({ jsonrpc:'2.0', id:3, method:'tools/call', params:{ name:'instructions/groom', arguments:{ mode:{ dryRun:true } } } })+'\n');
  await waitFor(out, o=> o.id===3 && o.result !== undefined);
    // remove
  proc.stdin?.write(JSON.stringify({ jsonrpc:'2.0', id:4, method:'tools/call', params:{ name:'instructions/remove', arguments:{ ids:['x1'], missingOk:true } } })+'\n');
